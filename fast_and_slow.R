...
d200<-function(r,m1=50,m2=50,s1=5,s2=5,n=2000){
	mu = c(m1, m2)  #平均ベクトルの定義
	sigma = matrix(c(s1^2, r*s1*s2, r*s1*s2, s2^2), 2, 2)#分散共分散行列の定義
	#r=(r*s1*s2)/(s1*s2)
	data = mvrnorm(n, mu, sigma)
	...
	#print(y)
	d <- n/2 #上位下位分岐点
	y1=y[1:d,]#COE能力下位半分の企業をセレクト
	y2=y[(d+1):n,]#COE能力上位半分の企業をセレクト
	#print(y1)
	#print(y2)
	out <- c()
	for(i in 1:100){
...
d22<-function(r,m1=50,m2=50,s1=5,s2=5,n=2000){
  mu = c(m1, m2)  #平均ベクトルの定義
  mysigma = matrix(c(s1^2, r*s1*s2, r*s1*s2, s2^2), 2, 2)#分散共分散行列の定義
  #r=(r*s1*s2)/(s1*s2)
  data = mvrnorm(n, mu, mysigma)
  #2次元正規乱数乱数sn個を生成  相関cor(exv[,1],exv[,2]) 
  y=data[data[,1]> m1, 2] #data.frameに格納しなくてもOK
  y2=subset(y, y>m2) 
  length(y2)/length(y)
}
...
d21<-function(r,m1=50,m2=50,s1=5,s2=5,n=2000){
  x <- 0;  y <- 1:n
  mu  <- c(m1, m2)  #平均ベクトルの定義
  mysigma <- matrix(c(s1^2, r*s1*s2, r*s1*s2, s2^2), 2, 2)#分散共分散行列の定義
  #r=(r*s1*s2)/(s1*s2)
  data1<-mvrnorm(n, mu, mysigma)
  #2次元正規乱数乱数sn個を生成  相関cor(exv[,1],exv[,2]) 
  data<-data.frame(ceo=data1[,1],cpfm=data1[,2]) #乱数の組をdata.frameに格納
  y=data[data$ceo > m1, 2]
  y2=subset(y, y>m2) 
  length(y2)/length(y)
}
